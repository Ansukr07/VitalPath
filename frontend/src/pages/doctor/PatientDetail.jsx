            ...
            <div className="main-content">
                ...
                <div className="page-body">
                    ...
                    {activeTab === 'triage' && triage && (
                        <div className="glass-card">
                            <h3 style={{ marginBottom: '1rem', fontSize: '0.85rem', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Active Triage Result</h3>

                            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.25rem' }}>
                                <div style={{ flex: 1, minWidth: 140, background: 'rgba(255,255,255,0.04)', borderRadius: '10px', padding: '0.85rem', textAlign: 'center' }}>
                                    <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', marginBottom: '0.25rem' }}>RULE ENGINE</div>
                                    <RiskBadge priority={triage.ruleEngine?.priority} />
                                    <div style={{ fontWeight: 700, fontSize: '0.85rem', marginTop: '0.35rem' }}>{triage.ruleEngine?.score}/100</div>
                                </div>
                                {triage.mlModel?.available && (
                                    <div style={{ flex: 1, minWidth: 140, background: 'rgba(255,255,255,0.04)', borderRadius: '10px', padding: '0.85rem', textAlign: 'center' }}>
                                        <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', marginBottom: '0.25rem' }}>ML MODEL</div>
                                        <RiskBadge priority={triage.mlModel?.priority} />
                                        <div style={{ fontWeight: 700, fontSize: '0.85rem', marginTop: '0.35rem' }}>{Math.round(triage.mlModel?.riskScore * 100)}%</div>
                                    </div>
                                )}
                                <div style={{ flex: 1, minWidth: 140, background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: '10px', padding: '0.85rem', textAlign: 'center' }}>
                                    <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', marginBottom: '0.25rem' }}>FINAL (MERGED)</div>
                                    <RiskBadge priority={triage.finalPriority} size="lg" />
                                    <div style={{ fontWeight: 800, fontSize: '0.9rem', marginTop: '0.35rem' }}>{triage.finalScore}/100</div>
                                </div>
                                {triage.doctorOverride?.isOverridden && (
                                    <div style={{ flex: 1, minWidth: 140, background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '10px', padding: '0.85rem', textAlign: 'center' }}>
                                        <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', marginBottom: '0.25rem' }}>DOCTOR OVERRIDE</div>
                                        <RiskBadge priority={triage.doctorOverride?.overriddenPriority} />
                                        <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.35rem' }}>{triage.doctorOverride?.reason}</div>
                                    </div>
                                )}
                            </div>

                            {triage.ruleEngine?.reasoning?.length > 0 && (
                                <>
                                    <div style={{ fontSize: '0.78rem', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.5rem', fontWeight: 700 }}>Rule Engine Flags</div>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginBottom: '1rem' }}>
                                        {triage.ruleEngine.reasoning.map((r, i) => (
                                            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', background: 'rgba(255,255,255,0.04)', borderRadius: '7px', padding: '0.55rem 0.9rem', fontSize: '0.82rem' }}>
                                                <span>{r.factor}</span>
